...
//give user type
//give user type
function give_user_type($email){
    $check_exist="SELECT IF(EXISTS(SELECT * FROM `vendor_users` WHERE `vendor_email`='$email'),1,0)as result;";
    if(check_if_exist($check_exist)){
        $select_query="SELECT user_type FROM `vendor_users` WHERE `vendor_email`='$email';";
...
// echo list_count('ananda.globaltech@gmail.com');
?>