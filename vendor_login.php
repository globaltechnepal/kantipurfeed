...
<body style="overflow-x: hidden;">
  ...
  if (isset($_POST['req_login'])) {
    ...
    if ($useremail == "" || $userpassword == "") {
      ...
      if ($result == 0) {
        ...
        $srv_email = '';
        $srv_pass = '';
        $myquery = "SELECT  `vendor_email`, `vendor_pass` FROM `vendor_users` WHERE `vendor_email` ='$useremail';"; //echo $myquery;
        // echo $myquery;
  ...
  <script>
  $.session.set('user','<?php echo $_SESSION['vendor_email']; ?>');
//   alert( $.session.get('user'));
      window.location.href = 'assets/vendor/purchase_history.php';
      </script>
